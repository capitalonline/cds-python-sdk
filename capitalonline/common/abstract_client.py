...
import capitalonline
from capitalonline.common.exception.cds_sdk_exception import CdsSDKException
from capitalonline.common.exception import CdsSDKException as SDKError
from capitalonline.common.http.request import ApiRequest
from capitalonline.common.http.request import RequestInternal
from capitalonline.common.profile.client_profile import ClientProfile
from capitalonline.common.sign import Sign
...
class AbstractClient(object):
    ...
    _endpoint = ''
    _service = ''
    _sdkVersion = 'SDK_PYTHON_%s' % capitalonline.__version__
    ...
    def _format_params(self, prefix, params):
        ...
        raise CdsSDKException("ClientParamsError", "some params type error")

    def _build_req_inter(self, action, params, req_inter):
        if self.profile.signMethod in ("HMAC-SHA1"):
            self._build_req_with_old_signature(action, params, req_inter)
        else:
            raise CdsSDKException("ClientError", "Invalid signature method.")
    ...
    def _build_req_with_old_signature(self, action, params, req):
        ...
        if self.credential.secret_id:
            params['AccessKeyId'] = self.credential.secret_id
        ...
        #url = Sign.get_signature(action, self.credential.secret_id, self.credential.secret_key, self.profile.signMethod, req.host, param=params)
    ...
    def _check_status(self, resp_inter):
        if resp_inter.status != 200:
            raise CdsSDKException("ServerNetworkError", resp_inter.data)

    def _percentEncode(self, s):
        if type(s) == bytes:
            res = urlparse.quote(s.decode(sys.stdin.encoding).encode('utf-8'), '')
        else:
            res = urlparse.quote(str(s).encode('utf-8'), '')
        res = res.replace('+', '%20')
        res = res.replace('*', '%2A')
        res = res.replace('%7E', '~')
        return res
    ...
    def _format_sign_string(self, params):
        ...
        canstring = ''
        for k, v in sortedD:
            canstring += '&' + self._percentEncode(k) + '=' + self._percentEncode(v)
        stringToSign = self.profile.httpProfile.reqMethod + '&%2F&' + self._percentEncode(canstring[1:])
        return stringToSign
    ...
    def call(self, action, params, headers=None):
        req = RequestInternal(self._get_endpoint(),
                              ...
                              self._requestPath,
                              header=headers)
        self._build_req_inter(action, params, req)
        resp_inter = self.request.send_request(req, params)
    ...
    def call_json(self, action, params, headers=None, options=None):
        ...
        """
        body = self.call(action, params, headers)
