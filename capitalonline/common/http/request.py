...
logger = logging.getLogger("cds_sdk_common")
...
class ApiRequest(object):
    ...
    def _request(self, req_inter, params=None):
        if self.keep_alive:
        ...
        if req_inter.method == 'GET':
            ...
            req_inter_url = '%s?%s' % (self.host, req_inter.data)
            return self.conn.request(req_inter.method, req_inter_url,
                                     params, req_inter.header)
        else:
            raise CdsSDKException(
                "ClientParamsError", 'Method only support (GET, POST)')

    def send_request(self, req_inter, params=None):
        try:
            http_resp = self._request(req_inter, params)
            ...
            raise CdsSDKException("ClientNetworkError", str(e))
